...
import { localMenus } from './routes/local';
...
async function generateAccess(options: GenerateMenuAndRoutesOptions) {
  ...
  return await generateAccessible(preferences.app.accessMode, {
    ...
    fetchMenuListAsync: async () => {
      ...
      // 后台返回路由/菜单
      const backMenus = await getAllMenusApi();
      // 转换为vben能用的路由
      const vbenMenus = backMenuToVbenMenu(backMenus);
      // 特别注意 这里要深拷贝
      const menus = [...cloneDeep(localMenus), ...vbenMenus];
      return menus;
    },
    // 可以指定没有权限跳转403页面
    forbiddenComponent,
